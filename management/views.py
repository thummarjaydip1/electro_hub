...
def index(request):
    if request.user.is_authenticated:
        ...
        year_total = Order.objects.filter(
        ...
        top_shell = Product.objects.annotate(
            total_qty = Sum('order__quantity')
        ).order_by("-total_qty")[:5]


        # chart last 5 day profit 
        # chart_order = (
        # Order.objects.values('order_date')
        # .annotate(day_total_profit=Sum('total_price'))
        # .order_by("-order_date")[:5]
        # )
        # chart_order = list(chart_order)
        # chart_order.reverse()

        # days = []
        # profit = []
        # for a in chart_order:
        #     day_name = a['order_date'].strftime("%d %b")
        #     days.append(day_name)
        #     profit.append(a['day_total_profit'])

        # plt.figure(figsize=(7,5))
        # plt.plot(days, profit, marker='o')
        # plt.xlabel("Days")
        # plt.ylabel("Profit")
        # plt.title("Last 5 Days Profit")
        # chart_path = os.path.join(settings.BASE_DIR,"management","static","chart.png")
    ...
    try:
        ...
        if search_category:
            category = category.filter(
                Q(name__contains= search_category)
            )
        ...
        if search_order:
            order = order.filter(
                Q(product_name__icontains = search_order)
            )
        return render(request,"dashboard/tables-basic.html",{
            "data":data,
            "category":category,
            "product":product,
            "order": order
...
def order_admin_edit_table(request,id):
    try:
        if request.user.role == "admin" or request.user.role == "staff":
            ...
            return redirect("error_page")
        return render(request,"dashboard/order_admin_edit_table.html",{"order":order})
    except:
